...
"SELECT title, (SUM(amount) - funding_goal) AS over_goal FROM projects INNER JOIN pledges ON projects.id = pledges.project_id GROUP BY projects WHERE over_goal > 0;"
end
